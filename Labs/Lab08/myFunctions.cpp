...
unsigned long min30 = ((2UL * 60) * 1000);
unsigned long min40 = ((4UL * 60) * 1000);

// Include the libraries we needed for temperature sensor
#include <OneWire.h>
...
File createCsvFile(String fileName,File csvFile) {
    ...
    if (csvFile) {
        csvFile.println("Timestamp,Voltage1,Temperature,Circuit");
        csvFile.close();
        return csvFile;
    } else {
        Serial.println("Error opening the file " + fileName);
